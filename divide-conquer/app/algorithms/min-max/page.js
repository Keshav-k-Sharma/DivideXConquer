...
import { generateMinMaxSteps } from '../../../lib/algorithms/minMax'
import MinMaxViz from '../../../components/visualizers/minMax'

const DEFAULT_ARRAY = [3, 41, 52, 26, 38, 57, 9, 49]
...
export default function MinMaxPage() {
  ...
  useEffect(() => {
    ...
    setSteps(generateMinMaxSteps(arr))
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <div className="mb-8">
        ...
        <h1 className="font-display text-4xl font-extrabold text-purple mb-2">Min & Max Finding</h1>
        <p className="text-muted text-sm">Find min and max simultaneously by splitting the array in half.</p>
      </div>

      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: 'Recurrence', value: 'T(n) = 2T(n/2) + O(1)' },
          { label: 'Time Complexity', value: 'O(n)' },
          { label: 'Comparisons', value: '3n/2 - 2' },
        ].map(({ label, value }) => (
      ...
      <div className="bg-surface border border-border rounded-xl p-4 mb-6">
        ...
        <input className="w-full bg-bg border border-border rounded-lg px-4 py-2 font-mono text-sm text-text focus:outline-none focus:border-purple"
          value={inputVal} onChange={e => setInputVal(e.target.value)} />
      </div>

      <div className="bg-surface border border-border rounded-xl p-6 mb-6">
        <MinMaxViz step={steps[current]} />
      </div>
